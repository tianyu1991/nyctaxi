...
png(file="PickuplocationEachHour",width=780,height=480)
...
png(file="MostPickuplocationEachHour",width=780,height=480)
...
##assume the index for velocity is 0.4
##assume the index for trip distance is 0.3
##assume 2,1 
##assume 
i_vel<-2
i_dis<-1
...
png(file="bestpickupcount",width=780,height=480)
...
png(file="averagespeed",width=780,height=480)
ggplot(s4,aes(mean_longitude,mean_latitude))+geom_point(aes(colour=mean_velocity),size=0.2,alpha=1/3)+scale_size_area()+ggtitle("average speed of trip from different pick up location at 6pm ")+xlab("longitude")+ylab("latitude")
dev.off()

png(file="bestpickup5002",width=780,height=480)
